...
def download_pdf(url, api_key):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Download error attempt {attempt + 1}: {str(download_err)}")
...
Be exhaustive and conservative. Use the most conservative figures across all analyses.
...
Produce the final merged Fundara underwriting report:"""
    ...
    try:
        ...
        print(f"Claude merge failed: {str(claude_err)}")
    ...
    try:
        ...
        print(f"OpenAI merge fallback failed: {str(openai_err)}")
    ...
    try:
        ...
        print(f"Grok merge fallback failed: {str(grok_err)}")

    # Last resort
    print("All merge attempts failed, returning best single analysis")
...
def run_analysis(data, contact_id, location_id, ghl_key, report_type):
    try:
        ...
        for i in range(1, 11):
            ...
            if url and url != "null" and url != "undefined" and url.startswith("http"):
        ...
        for idx, url in statement_urls:
            ...
            if pdf_bytes:
                ...
                if text:
                    ...
                    print(f"Warning: no text extracted from statement {idx}, skipping")
            else:
                print(f"Warning: could not download statement {idx}, skipping")

        if not combined_text.strip():
            print(f"Could not extract text from any PDFs for contact {contact_id}")
            return
        ...
        print(f"run_analysis error for contact {contact_id}: {str(e)}")
